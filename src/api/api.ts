...
function getToken(role: 'student' | 'librarian'): string {
    const token = localStorage.getItem('api_token' + role);
    return 'Bearer ' + token;
}

export function saveToken(role: 'student' | 'librarian', token: string) {
    localStorage.setItem('api_token' + role, token);
}

export function saveIdentity(role: 'student' | 'librarian', identity: string) {
...
export function saveRefreshToken(role: 'student' | 'librarian', token: string) {
